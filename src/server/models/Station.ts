...
import { ArrivalsResponse, DeparturesResponse } from '../controllers/Api';
...
export class Station {
    ...
    route ( req: any, res: { render: ( arg0: string, arg1: { arrivals: any; pageTitle: string; title: string } ) => void; } ) {
        fetch (
            `https://gateway.apiportal.ns.nl/reisinformatie-api/api/v2/arrivals?station=${req.params.code}`,
            {
                method: 'GET',
                headers: {
            ...
            .then ( ( json: { payload: {arrivals: []} } ) => {
                console.log ( json.payload.arrivals );
                res.render ( 'station', { title: 'NS info', pageTitle: 'NS', arrivals: json.payload.arrivals } );
            } );
    }
    // code: string;
